...
protected:
	...
	CCSprite *enemyBody;  // 展示怪物的精灵
	CCSize bodySize;      // 怪物的尺寸
	CCSpriteFrame *enemyLifeOver;  // 用来表示怪物挂掉的精灵帧
	CCSpriteFrame *overByArrow;    // 被箭射死的精灵帧

	CCPoint tileCoord;   // 用来表示怪物出生点的地图坐标系下的
	CCPoint enemyPos;    // GL坐标系下的，可以直接设置怪物的位置
...
private:
	CCSprite *enemyBody2;  // 另外两个火球精灵
	CCSprite *enemyBody3;

	CCSize fireSize;  // 单个火球的尺寸，在于主角的碰撞检测中这个是起主要检测作用的
	CCArray *pArrayFire;  // 三个火球的数组
...
private:
	...
	CCSpriteFrame *dark;   // 乌云掉落闪电的一瞬展示的
	CCSpriteFrame *normal; // 正常的白云
	void reSetNormal();
};
